    ...
    ui(new Ui::MainDialog)
{
    ...
    //获取本地IP地址，将其显示到组合框控件（下拉列表和下栏菜单功能）
    QString localIP=GetLocalIPAddressFunc();//本机IP
    ui->comboBox_ClientIP->addItem(localIP);
    ...
    platestr2.setColor(QPalette::WindowText,Qt::yellow);
    ...
    //设置字体颜色
    QPalette platestr3=ui->groupBox_2->palette();
    platestr3.setColor(QPalette::WindowText,Qt::yellow);
    ui->groupBox_2->setPalette(platestr3);
    //设置字体加粗
    QFont font3=ui->groupBox_2->font();
    font3.setBold(true);
    ui->groupBox_2->setFont(font3);


    //设置字体颜色
    QPalette platestr4=ui->groupBox_3->palette();
    platestr4.setColor(QPalette::WindowText,Qt::yellow);
    ui->groupBox_3->setPalette(platestr4);
    //设置字体加粗
    QFont font4=ui->groupBox_3->font();
    font4.setBold(true);
    ui->groupBox_3->setFont(font4);

    //设置字体颜色
    QPalette platestr5=ui->label_3->palette();
    platestr5.setColor(QPalette::WindowText,Qt::yellow);
    ...
    platestr6.setColor(QPalette::WindowText,Qt::yellow);
...
void MainDialog::OnNewConnectionFunc()
{
    ...
    if(P_TCPFileSocketObject->state()==QAbstractSocket::UnconnectedState)
    {
        ...
        connect(P_TCPFileSocketObject,SIGNAL(error(QAbstractSocket::SocketError)),this,SLOT(DisplayErrorInfoFunc(QAbstractSocket::SocketError)));
...
{
    //客户端断开连接时，则输出提示数据信息
    ui->plainTextEdit_MsgList->appendPlainText("[*****客户端已断开和服务器连接*****]");
...
void MainDialog::OnSocketReadyReadFunc()
{
    ...
    while (P_TCPTextSocketObject->canReadLine())
        ui->plainTextEdit_MsgList->appendPlainText("[客户消息"+strtemp+"]"+P_TCPTextSocketObject->readLine());
}
void MainDialog::OnFileClientConnectedFunc()
{
    ui->plainTextEdit_MsgList->appendPlainText("[*****客户端套接字已连接成功*****]");
}
...
void MainDialog::UpdateServerProgressFunc()
{
    ...
    if(m_FileBytesReceived<=sizeof(qint64)*2)
    {
        ...
        {
         in>>m_FileNames;
         ui->plainTextEdit_MsgList->appendPlainText(tr("[%1*****接收文件%2*****]").arg(strtemp).arg(m_FileNames));
    ...
    if(m_FileBytesReceived<m_FileTotalBytes)
    {
        ...
        m_InBlocks=P_TCPFileSocketObject->readAll();
    ...
    if(m_FileBytesReceived==m_FileTotalBytes)
    {
        ...
        m_FileNameSizes=0;
        m_LocalFiles->close();
        ui->plainTextEdit_MsgList->appendPlainText(QString("[%1*****接收文件%2成功*****]").arg(strtemp).arg(m_FileNames));
    }
}
void MainDialog::DisplayErrorInfoFunc(QAbstractSocket::SocketError socketError)
{
    P_TCPFileSocketObject->close();
...
{
    QString IP=ui->comboBox_ClientIP->currentText();
    quint16 port=ui->spinBox_ClientPort->value();
    QHostAddress addr(IP);

    P_TCPServerObject->listen(addr,port);
    ui->plainTextEdit_MsgList->appendPlainText("[*****服务器已开始监听*****]");
    ui->plainTextEdit_MsgList->appendPlainText("[IP Address:"+P_TCPServerObject->serverAddress().toString()+"]"+"[Port:"+QString::number(P_TCPServerObject->serverPort())+"]");
...
void MainDialog::on_pushButton_SendData_clicked()
{
    ...
    QString msg=ui->plainTextEdit_SendList->toPlainText();

    if(msg.isEmpty())
    {
        QMessageBox::critical(this,"错误","发送消息不能为空，请重新检查");
        ui->plainTextEdit_SendList->setFocus();
        return;
    }

    ui->plainTextEdit_MsgList->appendPlainText("[服务器消息"+strtemp+"]:"+msg);
    ui->plainTextEdit_SendList->clear();
